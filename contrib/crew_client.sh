...
client_help_msg() {
  echo "==> Configure the CREW_HOST environment variable or run $0 from a repository with a git remote named crew"
  echo "--> i.e. git remote add crew crew@<crew-host>:<app-name>"
  exit 20 # exit with specific status. only used in units tests for now
}

if [[ -z $CREW_HOST ]]; then
  if [[ -d .git ]] || git rev-parse --git-dir > /dev/null 2>&1; then
    CREW_HOST=$(git remote -v 2>/dev/null | grep -Ei "^crew" | head -n 1 | cut -f1 -d' ' | cut -f2 -d '@' | cut -f1 -d':' 2>/dev/null || true)
  else
    client_help_msg
  fi
fi
...
if [[ ! -z $CREW_HOST ]]; then
  _crew() {
    ...
    if [[ -d .git ]] || git rev-parse --git-dir > /dev/null 2>&1; then
      set +e
      appname=$(git remote -v 2>/dev/null | grep -Ei "crew@$CREW_HOST" | head -n 1 | cut -f1 -d' ' | cut -f2 -d':' 2>/dev/null)
      set -e
    else
    ...
    if [[ "$1" = "apps:create" ]] && [[ -z "$2" ]]; then
      ...
      if git remote add crew "crew@$CREW_HOST:$appname"
      then
        echo "-----> Crew remote added at $CREW_HOST"
        echo "-----> Application name is $appname"
      else
        echo "!      Crew remote not added! Do you already have a crew remote?"
        return
      fi
    ...
    ssh -t "crew@$CREW_HOST" $long_args "$verb" "$appname" "${args[@]}"
  }

  if [[ "$0" == "crew" ]] || [[ "$0" == *crew_client.sh ]] || [[ "$0" == $(which crew) ]]; then
    _crew "$@"
    exit $?
  fi
else
